...
class Keranjang extends API_WithoutAuth 
// class Keranjang extends API_Controller 
{
	private $master_table = 'keranjang';
    ...
    $offset = $page > 1 ? offset($limit, $page) : $offset;
		...
		$v_input_list = array('id_keranjang','id_user','id_sepatu','kuantitas');
      ...
      //    API RESPONSE - DELETE DATA
			case 'delete':
				...
				if ($cek_keberadaan_data > 0){
					$hapus_data = $this->m_keranjang->delete_data($and_where);
					if ($hapus_data){
						$api_status = TRUE;
						$api_code = 205;
						$api_message = http_status_code($api_code);
